"""
各モデルは、App実行の前にrun_example.shを一度実施しておかなくてはいけない
...
q.client.model_path
q.client.data_path_input
q.client.data_path_output
q.client.data_path_input_folder
"""

def run_python_scoring_pipeline():
    """ model_path上のPythonScoringPipelineを実行
    """
    #(todo) data_path_inputを指定して、スコアリング
...
async def serve(q: Q):
    ...
    if not q.client.model_selected:    # 初期画面。モデル選択前
        list_model_path = os.listdir(os.path.join('..', 'models'))
    ...
    await handle_on(q)     # 各ボタンが押された場合、発動
...
async def do_scoring(q: Q):
    ...
    # 解凍フォルダのパス
    q.client.data_path_input_folder = os.path.join(q.client.data_path_input, os.path.splitext(os.path.basename(local_path_zip))[0])
